...
public class IU {
	...
	static UIFunctions UserI= new UIFunctions();
	@SuppressWarnings("resource")
	public static void main(String args[]) throws IOException, InterruptedException {
		while (true) {
			Print.userCategorySelection();
			...
			while (true) {
				if (category.equals("1")) {
					...
					if (UserI.libraryInputCheck(inputString))
						break;
					int branch = Integer.parseInt(inputString);
					while (true) {
						Print.printLibraryOptions();
						String branchInput = in.nextLine();
						UserI.adminInputcheck(branchInput);
						
						if (branchInput.equals("1")) {
					...
					Print.adminSelections();
					String selectTable = in.nextLine();

					if (!UserI.adminInputcheck(selectTable))
						continue;
					...
					if (tableSelected == 1) {
						while (true) {
							...
							if (selectFunction.equals("1")) {
								...
								UserI.newBookInputCheck(bookIdInput);
								...
								BookController.addBooksToBookDataBase(Integer.parseInt(bookIdInput), titleInput, authId, pubId);
								// break;
